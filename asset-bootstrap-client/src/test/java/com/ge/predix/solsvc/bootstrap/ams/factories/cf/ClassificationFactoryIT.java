...
public class ClassificationFactoryIT extends BaseFactoryIT
{
    ...
    public void testCreateSimpleClassfication(List<Header> headers)
    {
        ...
        classification.setDescription("Creating classification GEMach1 for testing purpose");//$NON-NLS-1$
    ...
    public void testCreateClassfication(List<Header> headers)
    {
        ...
        classification.setDescription("Creating Turbine classification for testing purpose");//$NON-NLS-1$
        classification.setObsolete(false);
     ...
     * @param headers -
     */
    public void testCreateChildClassficationWithAttributesForGasTurbine(List<Header> headers)
    {
        Classification classification = new Classification();
        ...
        classification.setDescription("Creating GasTurbine classification for testing purpose");//$NON-NLS-1$
        classification.setObsolete(false);
    ...
    {
        Classification classification = this.classificationFactory.getClassification("Turbine", headers);//$NON-NLS-1$
    ...
    {
        Classification classification = this.classificationFactory.getClassification(uuid, headers);
        assertEquals("Creating Turbine classification w attributes for testing purpose--updated Descpt",//$NON-NLS-1$
    ...
    {
        Classification classification = this.classificationFactory.getClassification(uri,headers);
     ...
     * @param headers -
     */
    public void testGetChildClassifications(String parent, int expectedListSize, List<Header> headers)
    {
        List<Classification> list = this.classificationFactory.getClassificationsByFilter(parent,
                "showChildren", "999999",//$NON-NLS-1$ //$NON-NLS-2$
                headers);
     ...
     * @param headers -
     */
    public void testdeleteClassificationWhichHasChildren(String uuid, List<Header> headers)
    {
        // thrown.expect(AmsHttpClientErrorException.class);
        HttpResponse response = this.classificationFactory.deleteClassification(uuid, headers);
        Assert.assertEquals(response.getStatusLine().getStatusCode(),HttpStatus.SC_NO_CONTENT);
